...
public class ShortAnnotationValueHandler implements AnnotationValueHandler {
    ...
    public Object handle(Class<?> returnType, Expression expr, Map<String, String> importMap, String modelPackage) {
        if (expr.isCastExpr()) {
            ...
            if (castExpr.getType().isPrimitiveType()
                    && castExpr.getType().asPrimitiveType().getType() == Primitive.SHORT
                    && castExpr.getExpression().isIntegerLiteralExpr()) {
                return castExpr.getExpression()
                        .asIntegerLiteralExpr().asNumber().shortValue();
            }
        } else if (expr.isIntegerLiteralExpr()) {
            return expr.asIntegerLiteralExpr().asNumber().shortValue();
        }
        return null;
    }
}
