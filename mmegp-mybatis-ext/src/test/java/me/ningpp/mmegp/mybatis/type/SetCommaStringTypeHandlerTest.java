...
class SetCommaStringTypeHandlerTest extends BaseTypeHandlerTest {

    SetCommaStringTypeHandler typeHandler = new SetCommaStringTypeHandler();

    @Test
    void setParameterTest() throws SQLException {
        typeHandler.setParameter(ps, 1, new TreeSet<>(Set.of("str", "rstr")), null);
        verify(ps).setString(1, "rstr,str");
    }

    @Test
    public void getNullableResultByNameTest() throws SQLException {
        when(rs.getString("ids")).thenReturn(null);
        Set<String> ids = typeHandler.getResult(rs, "ids");
        assertTrue(ids.isEmpty());

        when(rs.getString("ids2")).thenReturn("");
        Set<String> ids2 = typeHandler.getResult(rs, "ids2");
        assertTrue(ids2.isEmpty());

        when(rs.getString("ids3")).thenReturn("a,b,c");
        Set<String> ids3 = typeHandler.getResult(rs, "ids3");
    ...
    public void getNullableResultByIndexTest() throws SQLException {
        ...
        Set<String> ids = typeHandler.getResult(rs, 1);
        assertTrue(ids.isEmpty());

        when(rs.getString(1)).thenReturn("");
        Set<String> ids2 = typeHandler.getResult(rs, 1);
        assertTrue(ids2.isEmpty());

        when(rs.getString(1)).thenReturn("a,b,c");
        Set<String> ids3 = typeHandler.getResult(rs, 1);
        assertArrayEquals(new String[]{"a", "b", "c"}, ids3.toArray(new String[0]));
    }
    ...
    public void getNullableResultCallableStatementTest() throws SQLException {
        ...
        Set<String> ids = typeHandler.getResult(cs, 1);
        assertTrue(ids.isEmpty());

        when(cs.getString(1)).thenReturn("");
        Set<String> ids2 = typeHandler.getResult(cs, 1);
        assertTrue(ids2.isEmpty());

        when(cs.getString(1)).thenReturn("a,b,c");
        Set<String> ids3 = typeHandler.getResult(cs, 1);
        assertArrayEquals(new String[]{"a", "b", "c"}, ids3.toArray(new String[0]));
    }
}
